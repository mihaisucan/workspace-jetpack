...
function evalForContext(aContext, aString, aCallback) {
  let window = getWindowForContext(aContext);
  let sandbox = new $sdk.Sandbox(window,
                {sandboxPrototype: window, wantXrays: false});

  sandbox.eval(aString, aCallback);
}

function inspectForContext(aContext, aString) {
  let window = getWindowForContext(aContext);
  let sandbox = new $sdk.Sandbox(window,
                {sandboxPrototype: window, wantXrays: false});

  sandbox.eval(aString, function(aError, aResult) {
    let propPanel = new $sdk.PropertyPanel();
    propPanel.open(aError || aResult, sandbox, aString);
  });
}
...
let panel = {
    ...
    switch (aMessage.action) {
      case "evalForContext":
        evalForContext(aMessage.context, aMessage.string,
                       aMessage.sendResult ?
                         this._sendEvalResult.bind(this, aMessage) : null);
        break;

      case "inspectForContext":
        inspectForContext(aMessage.context, aMessage.string);
        break;
  ...
  _sendEvalResult: function panel__sendEvalResult(aMessage, aError, aResult) {
    let result = {
      action: "evalResultForContext",
      context: aMessage.context,
      string: aMessage.string,
      meta: aMessage.meta,
      error: aError,
      result: aResult,
    };

    this._panel.postMessage(result);
  },
};
